import {Curtains, TextureLoader, Texture, Plane, ShaderPass} from '../../../../src/index.mjs';
...
window.addEventListener("load", function() {
    ...
    img.setAttribute("data-sampler", "uTestTexture");
    ...
    loader.loadImage(
        img,
        {}, // empty textures options
        (texture) => {
            newTexture = texture;
            //console.log("img loaded from asset loader", img);
            console.log("img loaded and texture created", newTexture);


            newTexture.onSourceUploaded(() => {
                console.log("texture uploaded to GPU", newTexture);

                if(shaderPass) {
                    newTexture.setMinFilter(curtains.gl.LINEAR);
                    newTexture.addParent(shaderPass);
                }
            });
        }, (img, error) => {
            console.log("there has been an error loading the image:", img, error);
        }
    );*/

    newTexture = new Texture(curtains, {
        sampler: "uTestTexture"
    });
    img.addEventListener("load", () => {
        newTexture.onSourceLoaded(() => {
            console.log("img loaded and texture created", newTexture);
        }).onSourceUploaded(() => {
            console.log("texture uploaded to GPU", newTexture);

            if(shaderPass) {
                newTexture.setMinFilter(curtains.gl.LINEAR_MIPMAP_NEAREST);
                newTexture.addParent(shaderPass);
            }
        });

        newTexture.setSource(img);
    });
    ...
    if(shaderPass) {
        ...
        shaderPass.onLoading((texture) => {
            console.log("shader pass image has been loaded and texture has been created:", texture, shaderPass);
        }).onReady(() => {
